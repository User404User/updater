...
use crate::network_client::{self as network, download_to_path_with_domain_replacement, patches_check_url, NetworkHooks, PatchCheckRequest};
...
#[cfg(test)]
pub use crate::network_client::{DownloadFileFn, Patch, PatchCheckRequestFn};
...
fn update_internal(_: &UpdaterLockState, channel: Option<&str>) -> anyhow::Result<UpdateStatus> {
    ...
    for event in events {
        let result = network::send_patch_event(event, &config);
...
pub fn report_launch_success() -> anyhow::Result<()> {
    ...
    with_config(|config| {
        ...
        std::thread::spawn(move || {
            ...
            let report_result = network::send_patch_event(event, &config_copy);
            if let Err(err) = report_result {
...
mod tests {
    ...
    fn events_sent_during_update() {
        ...
        use crate::events::{EventType, PatchEvent};
        use crate::network_client::PatchCheckResponse;
